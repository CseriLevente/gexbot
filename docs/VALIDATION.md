...
|---|---|---|---|
| Package version | `2.1.14` | `pyproject.toml` | anything ships |
...
| Manifest schema | `raw-capture-manifest/2.1.14` | `src/adapters/raw_store.py` | the *shape* of capture evidence changes |
...
| Operator-report schema | `raw-capture-run/2.1.14` | `src/tools/capture_thetadata_once.py` | the shape of the capture report changes |
| Run-intent schema | `raw-capture-intent/2.1.14` | `src/tools/capture_thetadata_once.py` | what a run states before its first request changes |
| HTTP-attempt schema | `http-attempt/2.1.14` | `src/adapters/http_attempts.py` | what is recorded about one request attempt changes |
