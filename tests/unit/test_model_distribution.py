...
def test_the_engine_version_reflects_this_release():
    assert MODEL_VERSION == "gex-engine/2.1.2"
...
def test_all_three_versions_are_documented():
    ...
    assert package["project"]["version"] == "2.1.2"

    text = (root / "docs" / "VALIDATION.md").read_text(encoding="utf-8")
    assert "gex-engine/2.1.2" in text
    assert "thetadata-v3-parser/2.1.1" in text or "thetadata-v3-parser/2.1.2" in text
