...
|---|---|---|---|
| Package version | `2.1.22` | `pyproject.toml` | anything ships |
...
| Operator-report schema | `raw-capture-run/2.1.22` | `src/tools/capture_thetadata_once.py` | the shape of the capture report changes |
...
| Pricing-evidence schema | `pricing-evidence/2.1.22` | `src/adapters/thetadata/live_behavior.py` | what a documented-versus-observed record must carry changes. New in v2.1.22: a v2.1.21 reader saw only the documented side and would report agreement where there is a conflict |
| Capture-certification schema | `capture-certification/2.1.22` | `src/adapters/thetadata/capture_certification.py` | what an offline certification report derives from a capture changes |
