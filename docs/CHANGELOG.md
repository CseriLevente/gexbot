...
## 2.1.23 - certification stops assuming the first capture
    ...
    v2.1.22 hypotheses      {4.2, 0.042}
    v2.1.22 ledger labels   "ACT_365", "16:00 America/New_York"

    planned capture #2      rate_value = 0.042

v2.1.22 reconstructed the first capture correctly and then wrote its answers
into the machinery that was supposed to find them. The planned second capture
sends a different wire value, so it would have been scored against the wrong
pair of hypotheses and labelled from the wrong literals -- a confident, wrong
certification rather than a failure.

**Status:** `IMPLEMENTED` | `TESTED_SYNTHETICALLY` |
`TESTED_WITH_OFFLINE_FIXTURES` | `VALIDATED_AGAINST_ONE_LIVE_THETADATA_CAPTURE` |
`NOT_READY_FOR_ANALYTICAL_DATASET`.

No capture was taken for this release. The repository remains incapable of
placing an order.

### Defects fixed

| S | Defect in v2.1.22 | Why it mattered | Fix |
|---|---|---|---|
| 1 | Inversion, rate hypotheses, day-count scoring, IV solve and ledger labels were all pinned to the first capture's 4.2/365/16:00 | A capture at any other rate would be misread and then mislabelled | Hypotheses derive from the captured wire value `w` (`DECIMAL -> w`, `PERCENT -> w/100`); rate and day count are searched jointly; labels come from the winning hypothesis |
| 2 | The rate could only have been generalised by adding a caller argument | That would let anyone pair one capture's responses with another capture's rate | The value is read from the run intent and *proved* by recomputing the request digest against the manifest's `planned_request_hash` |
| 3 | Documentation conflict and economic correctness were one fact | A corrected capture would inherit the first one's blocker permanently | `rate_units_documentation_live_conflict` and `capture_effective_rate_matches_intended_rate` are separate and separately reported |
| 4 | The synthetic test generated under the same constants the implementation contained | It proved the code agreed with itself | The fake vendor is parameterised; the matrix varies rate, unit, day count and close, and asserts on final evidence objects |
| 5 | `manifest_hash` was read from the file it was supposed to describe | A stored digest nothing recomputes is a label, not a check | Recomputed from the descriptors via `rebuilt_from`; any mismatch refuses |
| 6 | `archive_sha256 = supplied or capture.manifest_hash` | A field named after one artefact held the digest of another | Optional and empty when unknown; equality with the manifest hash is refused by the type |
| 7 | The 16:00 scope lived in a prose `scope` string | A downstream consumer had to parse English or assume a global rule | `expiration_clock_evidence` carries both regimes, the contradicting expirations by name, and an `OPEN` transition boundary |

### An adjacent defect, fixed because defect 5 needed it
...
The round trip was silently lossy, so recomputing the digest to verify a
manifest would have refused every honest capture and learned nothing about a
dishonest one. Same omission as v2.1.20's `ManifestRecord.of`, in the other
direction.

### The clock is read rather than matched
...
scored on a five-minute grid around that estimate.

The grid matters. The implied offset drifts with maturity -- 0.2490 at the
front, 0.2503 a week out -- so the median lands at 16:01 for a close that is
really 16:00, and 16:01 scores marginally better (3.10e-04 against 3.48e-04).
That gap is inside the noise a four-decimal `implied_vol` creates. The
unrounded estimate is kept in the evidence, so both numbers are visible.

### The rate and the day count could not be resolved in sequence

The inversion needs both before it can say anything about the clock, and the
clock decides the regimes. Read a 360-day vendor at 365 and every implied day is
1.4% long, which pushes whole-calendar-day expirations past the intraday
threshold. All eight combinations are reconstructed and the best fit wins; the
published tables are slices through that grid.

### The first capture is unchanged

`ACT_365`, 16:00 ET for the capture week, whole calendar days beyond, NBBO mid,
documentation conflict retained, still `ADAPTER_CERTIFICATION_EVIDENCE` and
still blocked -- now for the economic reason (`r=4.2` against an intended
`0.042`, a factor of 100) rather than for the documentation conflict.

`first_capture.json` was regenerated by the command, not edited.

### The dry run answers the question an operator is about to spend money on

    economic rate                       4.2%
    local model r                       0.042
    wire value                          0.042
    documented unit                     PERCENT_ANNUAL_RATE
    observed vendor unit                DECIMAL_ANNUAL_RATE
    predicted vendor effective r        0.042
    predicted matches intended          true
    documentation/live conflict         true
